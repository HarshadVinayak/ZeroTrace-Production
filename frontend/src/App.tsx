...
function AuthPage() {
  ...
  const simulateLogin = () => {
    setLoading(true);
    setTimeout(() => {
      localStorage.setItem("zt_auth", "token");
  ...
  return (
    <div className="min-h-screen bg-[#07131f] flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <motion.div initial={{opacity:0, scale:0.95}} animate={{opacity:1, scale:1}} className={`${GLASS_PANEL} max-w-sm w-full relative z-10 shadow-2xl`}>
         <div className="text-center mb-8">
            <div className="w-20 h-20 mx-auto mb-4 relative drop-shadow-[0_0_20px_rgba(0,255,159,0.5)]">
               <img src="/logo.png" alt="logo" className="w-full h-full object-contain" />
            </div>
            <h1 className="text-3xl font-display font-black text-white">ZeroTrace</h1>
            <p className="text-slate-400 font-medium text-sm mt-1">Intelligence for Sustainability</p>
         </div>
         <div className="flex gap-2 mb-6 bg-slate-800/50 p-1.5 rounded-xl border border-slate-700/50">
           <button onClick={()=>setMethod('google')} className={`flex-1 py-2 rounded-lg font-bold text-sm transition-all ${method==='google' ? 'bg-[#00ff9f]/20 text-[#00ff9f] shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}>Google</button>
           <button onClick={()=>setMethod('phone')} className={`flex-1 py-2 rounded-lg font-bold text-sm transition-all ${method==='phone' ? 'bg-[#00ff9f]/20 text-[#00ff9f] shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}>Phone OTP</button>
         </div>

         {method === 'google' ? (
           <button onClick={simulateLogin} disabled={loading} className="w-full flex items-center justify-center gap-3 bg-white hover:bg-slate-100 text-slate-900 py-3.5 rounded-xl font-extrabold transition-all shadow-md active:scale-95">
             {loading ? <Zap className="animate-spin w-5 h-5" /> : <svg className="w-5 h-5" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg>} Continue with Google
           </button>
         ) : (
           <div className="space-y-4">
             <input type="tel" placeholder="+1 (555) 000-0000" className="w-full bg-[#1e293b] text-white rounded-xl px-5 py-3.5 outline-none border border-slate-700 focus:border-[#00ff9f]/50 transition-colors font-medium" />
             <button onClick={simulateLogin} disabled={loading} className={`${BUTTON_PRIMARY} w-full justify-center py-3.5`}>
                {loading ? <Zap className="animate-spin w-5 h-5" /> : "Send OTP"}
             </button>
           </div>
         )}
         <p className="text-center text-slate-500 text-xs mt-6">Firebase Auth (Simulated)</p>
      </motion.div>
    </div>
  )
}
    ...
    <div className="min-h-screen bg-[#07131f] text-slate-300 font-sans selection:bg-teal-500/30 overflow-hidden flex">
      ...
      <aside className="w-80 h-screen border-r border-slate-800/60 bg-[#07131f]/90 backdrop-blur-3xl flex flex-col relative z-20 flex-shrink-0 animate-fade-in shadow-[4px_0_24px_rgba(0,0,0,0.5)]">
        ...
        <div className="p-4 border-t border-slate-800">
           <button onClick={() => { localStorage.removeItem("zt_auth"); window.location.reload(); }} className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-red-500/10 text-red-400 font-bold hover:bg-red-500/20 transition-all border border-red-500/20 active:scale-95">
             Log Out
           </button>
        </div>
      </aside>
...
export default function App() {
  ...
  const [isAuthenticated] = useState(!!localStorage.getItem("zt_auth"));
